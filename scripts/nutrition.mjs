export function makeNutritionHelpers(supabase) {
  ...
  async function fdcSearch(query, apiKey) {
    ...
    const url =
      ...
      new URLSearchParams({ api_key: apiKey, query, pageSize: "5" });

    const res = await fetch(url, {
      headers: { Accept: "application/json", "User-Agent": "Foodies/1.0" },
    });
  ...
  async function computeNutritionFromIngredients(
    ingredientGroups,
    servings,
    apiKey,
  ) {
    ...
    for (const ing of items) {
      ...
      if (!grams) {
        total.unknown.push({
          ...ing,
          reason: "Neviem prepočítať jednotku na gramy.",
        });
        continue;
      }
  ...
  return { computeNutritionFromIngredients };
}

function getSb(passed) {
  return passed ?? supabaseServer();
}
...
async function getPieceWeightGrams(ingredientNorm, supabase) {
...
async function getMappedFdcId(ingredientNorm, supabase) {
  const sb = getSb(supabase);
...
async function upsertMappedFdcId(ingredientNorm, fdcId, note, supabase) {
  const sb = getSb(supabase);
...
export async function computeNutritionFromIngredients(
  ...
  for (const ing of items) {
    ...
    const gramsPerPiece = await getPieceWeightGrams(nameNorm, supabase);
    const grams = toGrams(ing.amount, ing.unit, gramsPerPiece);
    ...
    let fdcId = await getMappedFdcId(nameNorm, supabase);
