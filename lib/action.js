...
export async function shareMeal(prevState, formData) {
  ...
  // --- price range (USD per serving) ---
  const priceMinRaw = formData.get("price_min_usd");
  const priceMaxRaw = formData.get("price_max_usd");

  const price_min_usd =
    priceMinRaw === null || String(priceMinRaw).trim() === ""
      ? null
      : Number(String(priceMinRaw).replace(",", "."));

  const price_max_usd =
    priceMaxRaw === null || String(priceMaxRaw).trim() === ""
      ? null
      : Number(String(priceMaxRaw).replace(",", "."));

  if (
    (price_min_usd !== null &&
      (!Number.isFinite(price_min_usd) || price_min_usd < 0)) ||
    (price_max_usd !== null &&
      (!Number.isFinite(price_max_usd) || price_max_usd < 0))
  ) {
    return { message: "Please enter a valid price range." };
  }
  ...
  if (
    ...
    return { message: "Min price cannot be higher than max price." };
  }
  ...
  if (apiKey) {
    try {
      nutrition = await computeNutritionFromIngredients(
        normalizedIngredientGroups,
        servings,
        apiKey,
      );
    } catch (e) {
  ...
  const meal = {
    ...
    nutrition_updated_at: nutrition ? new Date().toISOString() : null,
  };
