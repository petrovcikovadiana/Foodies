...
export async function getMeals({ category = "", q = "" } = {}) {
  ...
  if (category) {
    meals = meals.filter((m) => (m.categories || []).includes(category));
  }
...
export async function saveMeal(meal) {
  ...
  const payload = {
    ...
    price_min_usd: meal.price_min_usd ?? null,
    price_max_usd: meal.price_max_usd ?? null,

    nutrition_per_serving: meal.nutrition_per_serving ?? null,
    nutrition_per_recipe: meal.nutrition_per_recipe ?? null,
    nutrition_unknown: meal.nutrition_unknown ?? null,
    nutrition_updated_at: meal.nutrition_updated_at ?? null,
  };
